...
Los dos paradigmas principales en clasificación de imágenes son las arquitecturas basadas en Vision Transformers (ViT) y las Redes Neuronales Convolucionales (CNNs). 
...
Dentro de la familia de los Transformers, MaxViT (\textit{Multi-Axis Vision Transformer}) combina tanto mecanismos de atención global como convoluciones locales para capturar características a múltiples escalas \cite{tu2022maxvit}.
...
EfficientNetV2 es una evolución de la familia de EfficientNet (CNN) que optimiza tanto la arquitectura como el proceso de entrenamiento para mejorar la velocidad y precisión \cite{tan2021efficientnetv2}.
...
MobileNet es una arquitectura basada en CNN diseñada para dispositivos móviles y aplicaciones de \textit{edge computing} \cite{howard2017mobilenets}.
...
\begin{itemize}
   \item Rose Dataset: Consta de 2,725 imágenes y abarca 4 clases (Black Spot, Downy Mildew, Powdery Mildew y Healthy).
   \item Potato Dataset: Comprende 812 imágenes distribuidas en 3 clases (Early Blight, Late Blight, Healthy).
   \item Apple Dataset: Incluye 1582 imágenes que cubren 5 clases (Alternaria Spot, Brown Spot, Gray Spot, Rust, Healthy).
\end{itemize}
...
El dataset de manzanas requirió un preprocesamiento adicional, ya que sus anotaciones originales estaban en formato de máscaras de segmentación semántica en lugar de coordenadas de bounding boxes. 
Utilizando OpenCV (\texttt{cv2.findContours} y \texttt{cv2.boundingRect}), se convirtieron automáticamente las máscaras de las lesiones en \textit{bounding boxes}, generando las coordenadas necesarias para su integración en el flujo de trabajo de YOLO.
...
\begin{itemize}
    \item \textbf{Pipeline Especializado} La predicción del clasificador actúa como un enrutador dinámico. 
    ...
    \item \textbf{Pipeline Unificado}
    El clasificador identifica la especie, pero la detección se realiza mediante un único modelo YOLOv8 global entrenado con el dataset combinado de todas las especies y patógenos (Figura \ref{fig:pipeline_single}).
...
En términos de robustez inter-dominio, la versión 8 ofreció una mayor consistencia, superando a la v11 en el dataset de rosas (+1.5\% mAP) y manteniendo un empate técnico en patatas, mientras que las mejoras de la v11 en manzanas fueron marginales y no compensaron su inestabilidad en otros escenarios. 
...
En lo referente a la etapa de detección, la configuración del modelo \textbf{YOLOv8s} también se fundamentó en \textit{transfer learning} desde el dataset COCO. 
...
Como se puede observar en la Tabla \ref{tab:resultados_resumen}, los experimentos muestran una clara diferencia de rendimiento entre intentar detectar todo a la vez y dividir el problema por especies. 
El modelo \textbf{Monolítico}, que intenta aprender todas las enfermedades simultáneamente, tuvo bastantes dificultades, quedándose con un promedio de mAP50 de solo 0.66. 
El problema principal de este enfoque es que el modelo se confunde fácilmente ya que al tener que distinguir entre tantas texturas diferentes de hojas al mismo tiempo, termina cometiendo errores básicos, como detectar una enfermedad de rosa en una hoja de manzana. 
Esto se nota especialmente en las clases de Rosa y Manzana, donde los resultados fueron bastante bajos (0.56 y 0.63 respectivamente).

Al introducir la clasificación previa, los resultados mejoraron notablemente. 
El caso más evidente es el de las Rosas, donde el enfoque \textbf{Especializado} consiguió aumentar la precisión de 0.56 a casi 0.96. 
Además, no solo detecta mejor qué hojas están enfermas, sino que predice las \textit{bounding box} con mucha más exactitud; la métrica mAP50-95, se duplicó de 0.29 a 0.65.
Esto confirma que es mucho más fácil para el modelo aprender las formas complejas de una hoja de rosa si no tiene que preocuparse por las formas de las patatas o las manzanas al mismo tiempo.

El análisis comparativo entre los métodos \textbf{Unificado} y \textbf{Especializado} reveló que entrenar modelos expertos por especie es superior a aplicar filtros lógicos sobre un modelo generalista. 
Aunque ambos métodos comparten la etapa de clasificación inicial, solo el enfoque \textbf{Especializado} logró superar las limitaciones del modelo base en la detección de patata (0.82 vs 0.79), demostrando que la especialización mediante entrenamiento dedicado es más efectiva que el post-procesamiento de predicciones genéricas. 
Por tanto, el sistema \textbf{Especializado} emergió como la mejor opción con un mAP50 promedio de 0.90.


\section{Conclusiones}

El presente trabajo ha abordado el desafío de detectar enfermedades en múltiples tipos de cultivos, demostrando que el contexto biológico es una pieza fundamental que a menudo se ignora en los modelos de inteligencia artificial tradicionales. Nuestros experimentos validan la hipótesis inicial; dividir el problema en dos pasos jerárquicos (primero identificar la planta y luego buscar la enfermedad) es una estrategia muy superior a intentar resolverlo todo con un único modelo monolítico (\textit{end-to-end}).

Cuantitativamente, el enfoque jerárquico especializado no solo mejoró el rendimiento, sino que transformó la fiabilidad del sistema, elevando la precisión media (mAP50) de un modesto 0.6654 a un sobresaliente 0.9029. Este salto de calidad se vio claramente en el caso de las rosas, el cultivo más complejo visualmente, donde logramos duplicar la capacidad del modelo para localizar las lesiones con exactitud. Esto nos permite concluir que restringir el espacio de búsqueda elimina la confusión entre especies, permitiendo que cada detector se convierta en un verdadero experto en la morfología de su patógeno asignado, en lugar de ser un generalista propenso a errores.

Finalmente, este estudio también confirma la viabilidad técnica de la propuesta. Al integrar arquitecturas diseñadas para la eficiencia, como MobileNetV3 y YOLOv8s, hemos demostrado que es posible alcanzar niveles de precisión de laboratorio utilizando modelos ligeros y rápidos. Esto es crucial para la agricultura de precisión, ya que asegura que esta tecnología puede desplegarse en dispositivos móviles accesibles para los agricultores, sin depender de hardware costoso o conexiones a internet de alta velocidad.

\subsection{Líneas Futuras}

Una de las extensiones más inmediatas para este trabajo sería evaluar la escalabilidad de la arquitectura jerárquica ante un aumento significativo en la diversidad de cultivos. Actualmente, el sistema ha validado su eficacia con tres especies, pero un escenario agrícola real implica gestionar docenas de variedades. Sería interesante investigar si el clasificador MobileNetV3 mantiene su precisión al actuar como "enrutador" para 20 o 30 especies distintas, o si el aumento de clases comenzaría a introducir cuellos de botella que requieran una red más profunda. Ampliar el dataset permitiría confirmar si la estrategia de dividir el problema sigue siendo superior a los enfoques monolíticos a gran escala.
