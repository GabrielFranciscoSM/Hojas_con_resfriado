#!/usr/bin/env python3
"""
Script de clasificación de plantas usando MobileNetV3-Large.
Clasifica imágenes en 3 clases: manzana, patatas, rosas.

MobileNetV3-Large: ~5.4M parámetros (vs ~31M de MaxViT)
- Excelente balance entre tamaño y precisión
- Optimizado para dispositivos móviles/edge
- Rápido tanto en CPU como GPU
...
# ============================================================================
# MobileNetV3 usa entrada de 224x224 con normalización ImageNet
...
# ============================================================================
# BLOQUE 10: Función para crear modelo MobileNetV3
# ============================================================================
def create_mobilenet_model(num_classes, model_name='mobilenetv3_small_100.lamb_in1k', freeze_backbone=True):
    """
    Crea un modelo MobileNetV3-Small para fine-tuning.
    
    Comparativa de tamaños:
        - MobileNetV3-Small: ~2.5M parámetros  [ACTUAL]
        - MobileNetV3-Large: ~5.4M parámetros
        - EfficientNet-B0:   ~5.3M parámetros
        - MaxViT-Tiny:       ~31M parámetros
    
    Args:
        num_classes: Número de clases para clasificación
        model_name: Nombre del modelo preentrenado
        freeze_backbone: Si True, congela el backbone y solo entrena el clasificador
    ...
    print(f"Tamaño del modelo: ~{total_params * 4 / 1024 / 1024:.1f} MB (FP32)")
...
# ============================================================================
# BLOQUE 12: Entrenamiento - Fase 1 (solo clasificador)
# ============================================================================
print("=" * 60)
print("FASE 1: Entrenando solo el clasificador")
print("=" * 60)

model = create_mobilenet_model(num_classes=NUM_CLASSES, freeze_backbone=True).to(device)
...
optimizer = torch.optim.AdamW(model.parameters(), lr=1e-4)  # LR más bajo para MobileNet
...
MODEL_PATH = 'mobilenetv3_plant_classifier.pth'
save_model(
    model, 
    MODEL_PATH, 
...
# ============================================================================
# BLOQUE 17: Comparativa de modelos (informativo)
# ============================================================================
print("\n" + "=" * 60)
print("COMPARATIVA DE MODELOS")
print("=" * 60)
print("""
| Modelo             | Parámetros | Tamaño (FP32) | Velocidad | Precisión |
|--------------------|------------|---------------|-----------|-----------|
| MobileNetV3-Large  | ~5.4M      | ~22 MB        | ⚡⚡⚡      | ★★★★☆     |
| MobileNetV3-Small  | ~2.5M      | ~10 MB        | ⚡⚡⚡⚡     | ★★★☆☆     |
| EfficientNet-B0    | ~5.3M      | ~21 MB        | ⚡⚡⚡      | ★★★★☆     |
| EfficientNet-Lite0 | ~4.7M      | ~19 MB        | ⚡⚡⚡⚡     | ★★★★☆     |
| MaxViT-Tiny        | ~31M       | ~120 MB       | ⚡         | ★★★★★     |
""")
